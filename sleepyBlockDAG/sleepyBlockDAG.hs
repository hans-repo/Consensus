...
tickClientHandler (ClientTick tickTime) = do
    ...
    if V.length lastDeliveredOld > cmdRate*(length peers)
        ...
        then  lastDelivered .= lastXElements (cmdRate*(length peers)) lastDeliveredOld
        else return ()
...
isSubset smaller larger = V.any (`V.elem` largerIds) smallerIds
...
spawnServer batchSize clientPid = spawnLocal $ do
    ...
    runServer (ServerConfig myPid serverPids (Signature (show randomGen)) timeoutTicks tickTime [clientPid]) (ServerState 0 0 [genesisBlockSingle] [genesisBlockSingle] 0 timeInit batchSize randomGen 0)
